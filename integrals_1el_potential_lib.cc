...
#pragma omp for schedule(dynamic, 200)
		for(int i = 0; i < iterations; i++) {
			...
			for(unsigned int p = 0; p < psi_list.size(); p++) {
				...
				//TODO  Create special case if work.size == 0, if sum x, y, z == 0, if noOfContribs == 0
				for(int l = 0; l < num_mm_atoms; l++) {
					...
					for(unsigned int r = 0; r < work.size(); r++) {
						const int x = work[r].x;
						const int y = work[r].y;
						const int z = work[r].z;
						const qmmm_real factor  = work[r].factor;
