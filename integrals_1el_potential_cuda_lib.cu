...
/* The Atom is wrapped in a struct to make it possible to do 128 byte memcopy from global to on the GPU */
struct cuda_atom {
...
static bool is_pow_2(unsigned int x) {
	return ((x&(x-1))==0);
}

template <unsigned int block_size, bool nIsPow2>
__global__ void reduce_kernel(double *d_in, double *d_out, unsigned int n) {
	...
	if (block_size >= 512) { if (tid < 256) { smem[tid] = sum = sum + smem[tid + 256]; } __syncthreads(); }
	if (block_size >= 256) { if (tid < 128) { smem[tid] = sum = sum + smem[tid + 128]; } __syncthreads(); }
	if (block_size >= 128) { if (tid <  64) { smem[tid] = sum = sum + smem[tid +  64]; } __syncthreads(); }

	if (tid < 32) {
		volatile double* smemv = smem;
		if (block_size >=  64) { smemv[tid] = sum = sum + smemv[tid + 32]; __syncthreads(); }
		if (block_size >=  32) { smemv[tid] = sum = sum + smemv[tid + 16]; __syncthreads(); }
		if (block_size >=  16) { smemv[tid] = sum = sum + smemv[tid +  8]; __syncthreads(); }
		if (block_size >=   8) { smemv[tid] = sum = sum + smemv[tid +  4]; __syncthreads(); }
		if (block_size >=   4) { smemv[tid] = sum = sum + smemv[tid +  2]; __syncthreads(); }
		if (block_size >=   2) { smemv[tid] = sum = sum + smemv[tid +  1]; __syncthreads(); }
	}
